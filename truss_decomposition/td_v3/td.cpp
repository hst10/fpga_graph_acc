    ...
    if (ifs.is_open() && ifs.good())
    {
        ...
        while (std::getline(ifs, str))
        {
            if (!str.empty() && str[0] != '#')
            {
                ...
                int u, v; 
                ss >> u >> v; 
                tmp_edge_list.push_back(u); 
                tmp_edge_list.push_back(v); 
            }
        }
    }
    ifs.close();
...
void preprocess_graph_td(std::vector<int> &tmp_edge_list, 
                         ...
                         std::vector<int> &offset_list)
{
    ...
    for (int i = 0; i < tmp_edge_list.size(); i+=2)
    {
        int u = tmp_edge_list[i]; 
        int v = tmp_edge_list[i+1]; 
...
//  std::cout << "TARGET: " << node_a << ", " << node_b << std::endl;
    ...
    if (edge_not_found) // search in the whole edge list; not likely to be true
    {
        ...
        int end_small = 0; 
        int end_large = num_edges - 2; 
        int mid = (num_edges - 2)/2;
        if (mid % 2 == 1) mid = mid - 1; 
        while ((edge[mid] != node_a) || (edge[mid+1] != node_b))
        {
            if ((node_a > edge[mid]) || ((node_a == edge[mid]) && (node_b > edge[mid+1])))
                end_small = mid; 
            else
                end_large = mid; 

            mid = (end_small + end_large)/2;
            if (mid % 2 == 1) mid = mid - 1; 
        }
        assert((edge[mid] == node_a) && (edge[mid+1] == node_b));
        return (mid/2+1); 
    }
}

inline
int get_edge_idx_force(unsigned int node_a, unsigned int node_b, unsigned int* edge, unsigned int num_edges, unsigned int *hash_edge2idx)
{
//  std::cout << "TARGET: " << node_a << ", " << node_b << std::endl;
    unsigned int ht_idx = (unsigned int)(((node_a & 0x3ff) << 10) + (node_b & 0x3ff));
...
int main( int argc, char** argv )
{
    ...
    std::vector<int> tmp_edge_list, edge_list, neighbor_list, offset_list, triangle_list, triangle_list_offset;
    ...
    read_graph(argv[1], tmp_edge_list);

    auto t_file_done = std::chrono::high_resolution_clock::now();

    preprocess_graph_td(tmp_edge_list, edge_list, neighbor_list, offset_list);
    ...
    auto t_prep_select_done = std::chrono::high_resolution_clock::now();

    unsigned int *local_a = new unsigned int[BUF_SIZE];
    unsigned int *local_b = new unsigned int[BUF_SIZE];
/*    unsigned int local_a[BUF_SIZE];
    ...
    auto t_prep_hash_start = std::chrono::high_resolution_clock::now();
...
/*        else
    ...
    auto t_prep_hash_done = std::chrono::high_resolution_clock::now();
/*
...
*/
    ...
    if (edge_tc == nullptr)
    {
        ...
        std::cout << "edge_tc length = " << (edge_list.size()/2+1) << std::endl;
        exit(1);
    }
...
/*    delete [] local_a;
    ...
    auto t_init_tc_done = std::chrono::high_resolution_clock::now();
...
/*    int result = truss_decomposition(edge_tc, triangles, triangle_offsets, num_edges);
    ...
    accelerator acc; 
    acc.program("/home/xilinx/code/graph_challenge/td_v2/td_v2.4.bit");
...
//    int result = truss_decomposition(neighbors, offsets, edges, edge_tc, hash_edge2idx, edge_list.size(), offset_list.size(), neighbor_list.size());
    ...
    int final_result = acc.get_return();

    std::cout << "result = " << final_result << std::endl;

    std::chrono::duration<double> total_io_time = t_file_done - t_start; 
    std::chrono::duration<double> total_prep_select_time = t_prep_select_done - t_file_done; 
    std::chrono::duration<double> total_prep_hash_time = t_prep_hash_done - t_prep_hash_start; 
    std::chrono::duration<double> total_malloc_time = t_malloc_done - t_prep_hash_done; 
    std::chrono::duration<double> total_memcpy_time = t_memcpy_done - t_malloc_done; 
    std::chrono::duration<double> total_init_tc_time = t_init_tc_done - t_memcpy_done; 
    std::chrono::duration<double> total_program_time = t_program_done - t_program_start; 
    std::chrono::duration<double> total_kernel_time = t_acc_finish - t_acc_start; 
    std::chrono::duration<double> total_exec_time = t_acc_finish - t_start; 
    std::cout << "File IO time: " << total_io_time.count() << "s" << std::endl;
    std::cout << "Preprocessing pruning time: " << total_prep_select_time.count() << "s" << std::endl;
    std::cout << "Preprocessing hash time: " << total_prep_hash_time.count() << "s" << std::endl;
    std::cout << "CMA alloc time: " << total_malloc_time.count() << "s" << std::endl;
    std::cout << "Memcpy time: " << total_memcpy_time.count() << "s" << std::endl;
    std::cout << "Init TC time: " << total_init_tc_time.count() << "s" << std::endl;
    std::cout << "FPGA program time: " << total_program_time.count() << "s" << std::endl;
    std::cout << "Kernel exec time: " << total_kernel_time.count() << "s" << std::endl;
    std::cout << "Total exec time: " << total_exec_time.count() << "s" << std::endl;

    std::cout << "tc_summary, " << argv[1] << ", " << offset_list.size() - 2 << ", " << tmp_edge_list.size()/2 << ", " << final_result << ", "
          << total_io_time.count() << ", " << total_prep_select_time.count() << ", " << total_prep_hash_time.count() << ", " << total_malloc_time.count() << ", " 
          << total_memcpy_time.count() << ", " << total_init_tc_time.count() << ", " << total_program_time.count() << ", " << total_kernel_time.count() << ", " 
          << total_exec_time.count() << std::endl;
